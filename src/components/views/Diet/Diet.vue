<template>
  <div class="diet">
    ...
    <div
      class="diets"
      @touchstart="$touchstart($event,'diets')"
      @touchmove="$touchmove($event,'diets')"
      @touchend="$touchend($event,'diets')"
    >
      <dietcommon
        ref="dietcommon"
        :bottomColumn="true"
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      // 获取文章列表
      this.$refs.dietcommon.getList(3);
      // 获取轮播图
      this.$refs.dietcommon.getBanner(3);
    }, 100);
    window.addEventListener("scroll", this.onScroll);
  },
  methods: {
    ...
    onScroll() {
      ...
      var scrollHeight =
        document.querySelector(".diet").scrollHeight +
        document.querySelector("#app").scrollHeight;
      this.scrollTop = scrollTop;
      if (scrollTop + windowHeight >= scrollHeight - 100) {
        if (this.flag) {
          this.$refs.dietcommon.getMoresList(3);
        }
      }
      // 显示刷新按钮
