<template>
  <div class="healthy">
    ...
    <div
      class="healthys"
      @touchstart="$touchstart($event,'healthys')"
      @touchmove="$touchmove($event,'healthys')"
      @touchend="$touchend($event,'healthys')"
    >
      <dietcommon
        ref="dietcommon"
        :bottomColumn="true"
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      // 获取文章列表
      this.$refs.dietcommon.getList(6);
      // 获取轮播图
      this.$refs.dietcommon.getBanner(6);
    }, 100);
    window.addEventListener("scroll", this.onScroll);
  },
  methods: {
    ...
    onScroll() {
      ...
      var scrollHeight =
        document.querySelector(".healthy").scrollHeight +
        document.querySelector("#app").scrollHeight;
      this.scrollTop = scrollTop;
      if (scrollTop + windowHeight >= scrollHeight - 100) {
        if (this.flag) {
          this.$refs.dietcommon.getMoresList(6);
        }
      }
      // 显示刷新按钮
