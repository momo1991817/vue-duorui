<template>
  <div class="yoga">
    ...
    <div
      class="yogas"
      @touchstart="$touchstart($event,'yogas')"
      @touchmove="$touchmove($event,'yogas')"
      @touchend="$touchend($event,'yogas')"
    >
      <dietcommon
        ref="dietcommon"
        :bottomColumn="true"
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      // 获取文章列表
      this.$refs.dietcommon.getList(4);
      // 获取轮播图
      this.$refs.dietcommon.getBanner(4);
    }, 100);
    window.addEventListener("scroll", this.onScroll);
  },
  methods: {
    ...
    onScroll() {
      ...
      var scrollHeight =
        document.querySelector(".yoga").scrollHeight +
        document.querySelector("#app").scrollHeight;
      this.scrollTop = scrollTop;
      if (scrollTop + windowHeight >= scrollHeight - 100) {
        if (this.flag) {
          this.$refs.dietcommon.getMoresList(4);
        }
      }
      // 显示刷新按钮
