<template>
  <div class="emotion clearfix">
    ...
    <div
      class="emotions"
      @touchstart="$touchstart($event,'emotions')"
      @touchmove="$touchmove($event,'emotions')"
      @touchend="$touchend($event,'emotions')"
    >
      <dietcommon
        ref="dietcommon"
        class="dietcommon"
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      // 获取文章列表
      this.$refs.dietcommon.getList(2);
      // 获取轮播图
      this.$refs.dietcommon.getBanner(2);
    }, 100);
    window.addEventListener("scroll", this.onScroll);
  },
  methods: {
    ...
    onScroll() {
      ...
      var scrollHeight =
        document.querySelector(".emotion").scrollHeight +
        document.querySelector("#app").scrollHeight;
      this.scrollTop = scrollTop;
      if (scrollTop + windowHeight >= scrollHeight - 100) {
        if (this.flag) {
          this.$refs.dietcommon.getMoresList(2);
        }
      }
      // 显示刷新按钮
