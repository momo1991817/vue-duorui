<template>
  <div class="articleInfo-title">
    ...
    <!-- 遮罩层-->
    <div id="hidden" @click="goHidden" v-show="isFirst==1"></div>

    <!-- 发现更多-->
    <div class="find-more">
      <img src="../../../../static/android/waiting_found.png" id="wait-find" v-show="isFirst==1" />
      <img
        src="../../../../static/android/articleInfo_more.png"
        id="article-more"
        @click="goBackHome"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    goBackHome: function() {
      ...
      // 点击统计
      this.$http
        .get("/statistics/clicksStatistics?type=9&id=" + 1, {
    ...
    articles(item) {
      ...
      this.$router.push({
        ...
        query: { id: this.id, token: this.token }
      });
      // } else {
      //   var name = "duoruiapp_openInAppUrl";
      //   var data = {
      //     title: "多锐",
      //     url: this.$httpUrl + "articleInfo?id=" + item.id
      //   };
      //   bridge.callhandler(name, data, function(response) {});
      //   this.id = item.id;
      // }
    ...
    goHidden() {
      ...
      document.getElementById("wait-find").style.display = "none";
    },
    goSearch(item) {
      if (this.$header) {
        this.$router.push({
  ...
  mounted() {
    //
    if (this.$route.path == "/articleInfo-title") {
      var that = this;
      //监听窗口变化
